...
public class SimpleSearchHB extends HttpServlet implements Info {
	...
	private static final long serialVersionUID = 1L;
	   ...
	   void search(String keyword, HttpServletResponse response) throws IOException {
	      ...
	      try {
	         ...
	         if (keyword.isEmpty()) {
	            String selectSQL = "SELECT * FROM RecipeTable";
	            preparedStatement = connection.prepareStatement(selectSQL);
	         } else {
	            String selectSQL = "SELECT * FROM RecipeTable WHERE MYUSER LIKE ?";
   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      if (keyword != null && !keyword.isEmpty()) {
    	  listRecipes = UtilDBGeringer.listRecipe(keyword);
      } else {
    	  listRecipes = UtilDBGeringer.listRecipe();
      }
   ...
   void display(List<Recipe> listRecipes, PrintWriter out) {
      for (Recipe rec : listRecipes) {
         System.out.println("[DBG] " + rec.getId() + ", " //
        	       + rec.getType() + ", " //
                   + rec.getIngrediants() + ", " //
                   + rec.getDuration() + ", " //
                   + rec.getDirections());
         ...
         out.println("<li>" + rec.getId() + ", " //
        		   + rec.getType() + ", " //
                   + rec.getIngrediants() + ", " //
                   + rec.getDuration() + ", " //
                   + rec.getDirections()+ "</li>");
      }
   }
